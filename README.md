...
* Remove the -Dhz.jet.enabled=true argument as that is not documented in our install guide for HA, that was an informational message from Hazelcast, not a warning.
* Try to telnet between the two replicas using the hazelcast port number, which we can find in the logs or on disk in JENKINS_HOME/cloudbees-replication-discovery/*
  * `nc -vz <CONTROLLER_IP> 5701`
* Confirm that each node has the correct IP configured for -Dhudson.TcpSlaveAgentListener.hostName
* Add -Dhazelcast.diagnostics.enabled=true  to their JVM startup options
* Add a custom logger to the class com.cloudbees.jenkins.plugins.replication.hazelcast.FilesystemDiscoveryStrategy to debug

## Troubleshooting HA

* [Troubleshoot CloudBees CI on traditional platforms installation](https://docs.cloudbees.com/docs/cloudbees-ci/latest/troubleshooting/install-ha-active-active)
